...
export default function ProductCardLanding({
  img,
  title,
  platform,
  price,
  ...
  return (
    <>
      <motion.div
        ...
        className="main-card group relative">
        <div className="rounded-lg">
          <img loading="lazy" className="card-img" src={img} alt={title} />
        </div>
        ...
        <div className="flex flex-col justify-evenly flex-1">
          <Link
            to={`/product/${title}`}
            className="my-2  break-words hover:underline sm:h-14 h-auto ">
            {title}
          </Link>

          <p className="text-low-white flex gap-2 items-center">
            {platform == "PC" ? (
              <>
                <svg
          ...
          <div className="card-price-and-check ">
            <p className="price text-yellow-text text-lg">{price}$</p>
            {addTocartShow && (
              <AddToCartBtn
                card_img={img}
                platform={platform}
